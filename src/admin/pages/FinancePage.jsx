...
// Normalize and format employment type so freelancer values are preserved correctly
const normalizeEmploymentType = (type) => {
  if (type === null || type === undefined || type === '') return ''
  const value = String(type).trim().toLowerCase()

  if (['full_time', 'fulltime', 'full-time', 'full time'].includes(value)) return 'full_time'
  if (['freelancer', 'freelance', 'contractor', 'contract'].includes(value)) return 'freelancer'

  return value
}

const getInspectorEmploymentType = (inspector) => {
  if (!inspector) return ''

  return normalizeEmploymentType(
    inspector.employment_type ?? inspector.employmentType ?? inspector.category ?? inspector.inspector_category ?? ''
  )
}

const formatEmploymentType = (type) => {
  const normalizedType = normalizeEmploymentType(type)
  if (normalizedType === 'freelancer') return 'Freelancer'
  if (normalizedType === 'full_time') return 'Full Time'
      ...
      <ReasonDialog
        ...
        fields={[
          {
            ...
            options: inspectors.map(i => ({ 
              value: i.user_id, 
              label: `${i.name} (${i.user_id}) - ${formatEmploymentType(getInspectorEmploymentType(i))}` 
            })),
            onChange: (value, next) => {
              ...
              return {
                ...next,
                inspector_category: formatEmploymentType(getInspectorEmploymentType(selectedInspector)),
              }
            },
          },
          {
        ...
        onSubmit={async (form) => {
          try {
            ...
            const inspectorCategory = getInspectorEmploymentType(selectedInspector)
            if (inspectorCategory) {
              payload.inspector_category = inspectorCategory
            }
        ...
        onFormChange={(form, setForm) => {
          if (form.inspector_id) {
            const selectedInspector = inspectors.find(i => i.user_id === form.inspector_id)
            if (selectedInspector) {
              const formattedCategory = formatEmploymentType(getInspectorEmploymentType(selectedInspector))
