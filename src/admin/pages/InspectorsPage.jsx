...
export function InspectorsPage() {
  ...
  useEffect(() => {
    document.title = 'Inspector Onboarding · CARNALYSYS'
  }, [])
      ...
      <ReasonDialog
        ...
        onSubmit={async (form) => {
          try {
            ...
            // Validate start_time and end_time for present and half_day
            if (['present', 'half_day'].includes(form.availability_status)) {
              if (!form.start_time) {
                showSnack({ tone: 'danger', title: 'Error', message: 'Start Time is required for Present/Half Day' })
                return
              }
              if (!form.end_time) {
                showSnack({ tone: 'danger', title: 'Error', message: 'End Time is required for Present/Half Day' })
                return
              }
              // Validate that start_time is before end_time
              if (form.start_time && form.end_time && form.start_time >= form.end_time) {
                showSnack({ tone: 'danger', title: 'Error', message: 'End Time must be after Start Time' })
                return
              }
            }
      ...
      <ReasonDialog
        ...
        onSubmit={async (form) => {
          try {
            ...
            if (['present', 'half_day'].includes(form.availability_status)) {
              if (!form.start_time) {
                setAvailabilityFieldErrors({ start_time: 'Start Time is required for Present/Half Day' })
                return
              }
              if (!form.end_time) {
                setAvailabilityFieldErrors({ end_time: 'End Time is required for Present/Half Day' })
                return
              }
              // Validate that start_time is before end_time
              if (form.start_time && form.end_time && form.start_time >= form.end_time) {
                setAvailabilityFieldErrors({ end_time: 'End Time must be after Start Time' })
                return
              }
            }
            ...
            const payload = {
              ...
              availability: dates.map((date) => {
                ...
                if (['present', 'half_day'].includes(form.availability_status)) {
                  availabilityEntry.start_time = form.start_time
                  availabilityEntry.end_time = form.end_time
                }
