...
import { createPDIRequest, listPDIRequests, getPDIRequestById, assignInspector, confirmManualPayment, confirmManualRemainingPayment, createPaymentLink, getPaymentStatus, verifyPaymentLink, deletePdiRequest, refundPdiRequest, updatePdiBookingStatus, updatePdiPaymentStatus, overrideRemainingPayment } from '../../api/inspection'
...
export function NewInspectionPage() {
  ...
  // State for Adjust Payment (override remaining payment) modal
  const [showAdjustPaymentModal, setShowAdjustPaymentModal] = useState(false)
  const [adjustPaymentRequestId, setAdjustPaymentRequestId] = useState(null)
  const [adjustPaymentCurrentRemainingPaise, setAdjustPaymentCurrentRemainingPaise] = useState(0)
  const [adjustPaymentRemainingAmount, setAdjustPaymentRemainingAmount] = useState('')
  const [adjustPaymentReason, setAdjustPaymentReason] = useState('')
  ...
  // Handler for adjust (override) remaining payment
  const handleAdjustPayment = async () => {
    if (adjustPaymentRemainingAmount === '') { alert('Remaining payment is required.'); return }
    const remainingAmountPaise = Math.round(parseFloat(adjustPaymentRemainingAmount) * 100)
    if (Number.isNaN(remainingAmountPaise) || remainingAmountPaise < 0) {
      alert('Remaining payment must be a valid amount.')
      return
    }
    if (remainingAmountPaise > adjustPaymentCurrentRemainingPaise) {
      alert('Remaining payment cannot be greater than the current remaining amount.')
      return
    }
    setAdjustPaymentLoading(true)
    try {
      await overrideRemainingPayment(adjustPaymentRequestId, remainingAmountPaise, adjustPaymentReason.trim())
      alert('Remaining payment adjusted successfully!')
      setShowAdjustPaymentModal(false)
      setAdjustPaymentRemainingAmount('')
      setAdjustPaymentReason('')
      refreshPDIRequests()
    } catch (error) {
      alert(error?.detail || error?.message || 'Adjustment failed. Please try again.')
    } finally {
      setAdjustPaymentLoading(false)
    }
  }
  ...
  return (
    <div className="space-y-3">
            ...
            {actionsMenu.requestId && (
              <>
                ...
                <button
                  ...
                  onClick={() => {
                    const reqId = actionsMenu.requestId
                    const pdiRequest = pdiRequests.find(p => p.request_id === reqId)
                    const currentRemainingPaise = pdiRequest?.remaining_amount_paise || 0
                    setAdjustPaymentRequestId(reqId)
                    setAdjustPaymentCurrentRemainingPaise(currentRemainingPaise)
                    setAdjustPaymentRemainingAmount((currentRemainingPaise / 100).toFixed(2))
                    setAdjustPaymentReason('')
      ...
      {/* Adjust Payment (Override Remaining Payment) Modal */}
      {showAdjustPaymentModal && (
        <div className="fixed inset-0 z-50">
          <div className="absolute inset-0 bg-black/30" onClick={() => setShowAdjustPaymentModal(false)} />
          <div className="absolute left-1/2 top-1/2 w-[92vw] max-w-md -translate-x-1/2 -translate-y-1/2 rounded-xl border border-slate-200 bg-white shadow-lg">
            <div className="relative border-b border-slate-200 px-4 py-3">
              <h3 className="text-lg font-semibold text-slate-900">Adjust Payment</h3>
              <Button variant="icon" size="icon" className="absolute right-2 top-2" onClick={() => setShowAdjustPaymentModal(false)}>
                <X className="h-4 w-4" />
              </Button>
            </div>
            <div className="p-4 space-y-4">
              <div className="rounded-lg border border-slate-200 bg-slate-50 p-3 text-sm flex justify-between">
                <span className="text-slate-500">Current Remaining Amount</span>
                <span className="font-medium text-slate-800">₹{(adjustPaymentCurrentRemainingPaise / 100).toFixed(2)}</span>
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Remaining Payment <span className="text-red-500">*</span></label>
                <Input
                  type="number"
                  min="0"
                  max={adjustPaymentCurrentRemainingPaise / 100}
                  step="0.01"
                  value={adjustPaymentRemainingAmount}
                  onChange={(e) => setAdjustPaymentRemainingAmount(e.target.value)}
                  placeholder="Enter new remaining payment amount"
                  className="w-full"
                />
                <p className="mt-1 text-xs text-slate-500">Can only be lowered, up to the current remaining amount.</p>
              </div>
              <div>
                <label className="block text-sm font-medium text-slate-700 mb-1">Reason</label>
                <Input value={adjustPaymentReason} onChange={(e) => setAdjustPaymentReason(e.target.value)} placeholder="Enter reason (optional)" className="w-full" />
              </div>
              <div className="flex gap-3 justify-end pt-2">
                <Button variant="secondary" onClick={() => setShowAdjustPaymentModal(false)} disabled={adjustPaymentLoading}>Cancel</Button>
                <Button variant="primary" onClick={handleAdjustPayment} disabled={adjustPaymentLoading}>
                  {adjustPaymentLoading ? 'Adjusting...' : 'Adjust Payment'}
                </Button>
              </div>
            </div>
          </div>
