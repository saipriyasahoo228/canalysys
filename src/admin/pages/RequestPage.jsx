...
const RequestPage = () => {
  ...
  // Pagination states for Register Customers (client-side, fixed 10 rows per page)
  ...
  // Reset to page 1 when Registered Customers filters change (client-side)
  useEffect(() => {
    setCurrentPageCustomer(1);
  }, [customerSearchTerm, customerStartDate, customerEndDate]);
  ...
  // Fetch Registered Customers. The API doesn't actually honor `page`/`limit` (it
  // always returns the full list), so pagination for this tab is done client-side
  // below, the same way the other two tabs paginate their already-fetched data.
  const fetchRegisteredCustomers = async () => {
    try {
      ...
      setTotalCustomers(response.total_count ?? (response.customers || []).length);
  ...
  // Handle customer page changes (client-side, since the API always returns the full list)
  const handleNextPage = () => {
    setCurrentPageCustomer((prev) => Math.min(prev + 1, totalCustomerPages));
  };

  const handlePrevPage = () => {
    setCurrentPageCustomer((prev) => Math.max(prev - 1, 1));
  };

  const handlePageChangeCustomer = (page) => setCurrentPageCustomer(page);
  ...
  const currentCustomerData = filteredCustomers.slice(
    (currentPageCustomer - 1) * ITEMS_PER_PAGE,
    currentPageCustomer * ITEMS_PER_PAGE
  );
  ...
  // Register Customers Table (client-side pagination, 10 rows per page)
            ...
            <td colSpan="5" className="px-4 py-3">
              <div className="flex flex-col sm:flex-row justify-between items-center gap-3">
                <div className="text-sm text-gray-500">
                  ...
                  {Math.min(currentPageCustomer * ITEMS_PER_PAGE, filteredCustomers.length)} of {filteredCustomers.length} entries
                </div>
                
                {totalCustomerPages > 1 && (
                  <div className="flex items-center gap-1">
                    <button onClick={() => handlePageChangeCustomer(1)} disabled={currentPageCustomer === 1} className="px-2 py-1 rounded-md text-sm bg-white border border-gray-300 text-gray-600 hover:bg-gray-50 disabled:opacity-50">First</button>
                    <button onClick={handlePrevPage} disabled={currentPageCustomer === 1} className="px-2 py-1 rounded-md text-sm bg-white border border-gray-300 text-gray-600 hover:bg-gray-50 disabled:opacity-50">Prev</button>
                    {getPageNumbers(currentPageCustomer, totalCustomerPages).map(page => (
                      <button key={page} onClick={() => handlePageChangeCustomer(page)} className={`px-2 py-1 rounded-md text-sm ${currentPageCustomer === page ? 'bg-blue-600 text-white' : 'bg-white border border-gray-300 text-gray-600 hover:bg-gray-50'}`}>{page}</button>
                    ))}
                    <button onClick={handleNextPage} disabled={currentPageCustomer === totalCustomerPages} className="px-2 py-1 rounded-md text-sm bg-white border border-gray-300 text-gray-600 hover:bg-gray-50 disabled:opacity-50">Next</button>
                    <button onClick={() => handlePageChangeCustomer(totalCustomerPages)} disabled={currentPageCustomer === totalCustomerPages} className="px-2 py-1 rounded-md text-sm bg-white border border-gray-300 text-gray-600 hover:bg-gray-50 disabled:opacity-50">Last</button>
                  </div>
                )}
              </div>
            </td>
